...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/portage-2.2.8-r1.ebuild,v 1.7 2014/03/14 10:51:18 ago Exp $
...
SLOT="0"
IUSE="build doc epydoc +ipc linguas_ru pypy2_2 python2 python3 selinux xattr"
...
python_dep_ssl="python3? ( =dev-lang/python-3*[ssl] )
	!pypy2_2? ( !python2? ( !python3? (
		|| ( >=dev-lang/python-2.7[ssl] dev-lang/python:2.6[threads,ssl] )
	) ) )
	pypy2_2? ( !python2? ( !python3? ( virtual/pypy:2.0[bzip2] ) ) )
...
python_dep="${python_dep}
	python_targets_pypy2_2? ( virtual/pypy:2.0 )
...
RDEPEND="${python_dep}
	...
	xattr? ( kernel_linux? (
		$(for python_impl in python{2_6,2_7,3_2} pypy2_2; do
...
pkg_setup() {
	...
	if use pypy2_2 && use python3 ; then
		ewarn "Both pypy2_2 and python3 USE flags are enabled, but only one"
		ewarn "can be in the shebangs. Using python3."
	fi
	if use pypy2_2 && use python2 ; then
		ewarn "Both pypy2_2 and python2 USE flags are enabled, but only one"
		ewarn "can be in the shebangs. Using python2"
	fi
	if ! use pypy2_2 && ! use python2 && ! use python3 && \
